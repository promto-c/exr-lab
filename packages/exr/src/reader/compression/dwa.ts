...
function uncompressZipStyle(
  ...
  const raw =
    expectedUncompressedSize > 0
      ? unzlibSync(compressed, { out: inflateOut })
...
function lossyDctDecode(
  ...
  const outWords = new Uint16Array(
    outBuffer.buffer,
    outBuffer.byteOffset,
    outBuffer.byteLength >> 1,
  );
  ...
  for (let comp = 0; comp < numComp; comp++) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        dataView.setFloat32(
          offset + x * INT16_SIZE * sampleWords,
          float16ToFloat32(outWords[srcWords]),
          true,
        );
...
function lossyDctChannelDecode(
  ...
  if (channel.pixelType === 2) {
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        dataView.setFloat32(
          offset + x * INT16_SIZE * channel.sampleWords,
          float16ToFloat32(outWords[srcWords]),
          true,
        );
...
function decodeDwaChunk(context: DwaDecodeContext): Uint8Array {
  ...
  for (const channel of context.part.channels) {
    ...
    if (xSampling !== 1 || ySampling !== 1) {
      throw new Error(
        'DWAA/DWAB decoding currently requires xSampling=1 and ySampling=1 for all channels.',
      );
    }
  }
    ...
    switch (header.acCompression) {
      case STATIC_HUFFMAN: {
        ...
        hufUncompress(
          ...
          view,
          cursor,
          header.acCompressedSize,
          acBuffer,
          header.totalAcUncompressedCount,
        );
      ...
      case DEFLATE: {
        ...
        const decodedView = new Uint16Array(decoded.buffer, decoded.byteOffset, decodedWords);
        acBuffer = new Uint16Array(decodedWords);
        acBuffer.set(decodedView);
  ...
  if (header.dcCompressedSize > 0) {
    ...
    const dcView = new Uint16Array(dcBytes.buffer, dcBytes.byteOffset, dcWords);
    dcBuffer = new Uint16Array(dcWords);
    dcBuffer.set(dcView);
  ...
  if (header.rleRawSize > 0) {
    ...
    const decodedRle = decodeRunLength(decoded, header.rleRawSize);
    rleBuffer = new Uint8Array(decodedRle.byteLength);
    rleBuffer.set(decodedRle);
  ...
  for (let i = 0; i < channelData.length; i++) {
    ...
    switch (channel.compression) {
      ...
      case LOSSY_DCT: {
        const usage = lossyDctChannelDecode(
          i,
          rowOffsets,
          channelData,
          acBuffer,
