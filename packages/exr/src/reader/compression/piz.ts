...
function countSampledRowsInRange(
  start: number,
  end: number,
  sampleOrigin: number,
  sampling: number,
...
function buildChannelLayout(
  part: ExrPart,
  chunkY: number,
  linesInChunk: number,
): { channels: PizChannelLayout[]; totalWords: number } {
  if (!part.dataWindow) {
...
function decodePizChunk(context: PizDecodeContext): Uint8Array {
  ...
  const { channels, totalWords } = buildChannelLayout(
    context.part,
    context.chunkY,
    context.linesInChunk,
  );
