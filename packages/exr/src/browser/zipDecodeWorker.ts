...
scope.onmessage = (event: MessageEvent<ZipDecodeRequest>) => {
  ...
  try {
    ...
    const raw =
      message.expectedUncompressedSize > 0
        ? unzlibSync(compressed, { out: ensureInflateScratch(message.expectedUncompressedSize) })
    ...
    scope.postMessage(response, [decoded.buffer]);
  } catch (error) {
    ...
    scope.postMessage(response as ZipDecodeResponse);
  }
};
