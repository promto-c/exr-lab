...
import { parseExr } from '@blackboard/exr-reader';
...
export class PrefetchEngine {
  ...
  private async prefetchSingleFrame(frame: PrefetchFrame, gen: number): Promise<void> {
    ...
    try {
      ...
      const decoded = await decodeExrPartWithWorkers(buffer, structure, { partId });
      const rawResult = toRawDecodeResult(decoded, part);
      if (gen !== this.generation) return;
