...
export const SidebarLayout: React.FC<SidebarLayoutProps> = ({
  ...
  const handleSplitterPointerMove = React.useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      const minCurrent =
        (panes[index].minSize ?? DEFAULT_MIN_PANE_SIZE) / dragState.availableHeight;
      const minNext =
        (panes[index + 1].minSize ?? DEFAULT_MIN_PANE_SIZE) / dragState.availableHeight;
  ...
  const startResize = (splitterIndex: number) => (event: React.PointerEvent<HTMLDivElement>) => {
  ...
  const renderPane = (pane: SidebarPane) => {
    ...
    const content =
      React.isValidElement(pane.content) && pane.collapsed === undefined
        ? React.cloneElement(
            pane.content as React.ReactElement<{
              collapsed?: boolean;
              onCollapsedChange?: (collapsed: boolean) => void;
            }>,
            {
    ...
    const mobileFlexClass = isMobile ? (paneCollapsed ? 'flex-[0_0_auto]' : 'flex-1') : '';
  ...
  return (
    <div className={cx('h-full flex flex-col', className)}>
      ...
      <div
        ref={bodyRef}
        className={cx('flex-1 min-h-0', isMobile ? 'flex flex-col overflow-y-auto' : 'grid')}
