...
import type { PlaybackMode } from '../features/sequence/transport';
...
const PLAYBACK_MODES: { value: PlaybackMode; label: string; description: string }[] = [
  ...
  },
  {
    ...
    label: 'Every frame',
    description:
      'Play every frame in order. May slow down if decode takes longer than frame interval.',
  },
  {
    value: 'cached',
    label: 'Wait for cache',
    description:
      'Only advance once the next frame is fully decoded. Guarantees no skip or stutter.',
  },
];
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        ...
        <div className="rounded-lg border border-neutral-800 bg-neutral-900/60 px-3 py-2">
          ...
          <div className="font-mono text-neutral-200">
            {formatBytes(cacheStats.uniqueCacheBytes)}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {cacheExceeded && (
          <span className="text-[10px] text-red-400">
            Cache is above the limit and will auto-trim.
          </span>
        )}
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="border-t border-neutral-800 pt-3 text-xs text-neutral-500">
        <div className="flex flex-wrap items-center gap-3">
          <span>
            Part cache: {formatBytes(cacheStats.partCacheBytes)} · {cacheStats.partCacheCount}
          </span>
          <span>
            Frame cache: {formatBytes(cacheStats.frameCacheBytes)} · {cacheStats.frameCacheCount}
          </span>
          <span>
            Buffer cache: {formatBytes(cacheStats.bufferCacheBytes)} · {cacheStats.bufferCacheCount}
          </span>
        </div>
      </div>
    </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-neutral-500 ml-1">
              parallel decode{prefetchConcurrency > 1 ? 's' : ''}
            </span>
          </div>
        </div>
      )}
