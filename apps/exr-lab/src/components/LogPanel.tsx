...
const StatusIcon = ({ status }: { status: LogStatus }) => {
  switch (status) {
    ...
    default:
      return <Activity className="w-4 h-4 text-teal-400 animate-pulse" />;
  }
};
...
function toTableData(value: unknown): DetailTableData | null {
  if (!isRecord(value)) return null;
  const attributesRaw = value.attributes;
  if (!Array.isArray(attributesRaw)) return null;

  const attributes: DetailAttribute[] = attributesRaw.filter(isRecord).map((row) => ({
    key: typeof row.key === 'string' ? row.key : '',
    val: typeof row.val === 'string' ? row.val : String(row.val ?? ''),
  }));

  const channelsRaw = value.channels;
  const channels = Array.isArray(channelsRaw)
    ? channelsRaw.filter(isRecord).map((channel) => ({
...
function toChunkMapData(value: unknown): ChunkMapData | null {
  if (!isRecord(value)) return null;
...
const DetailView = ({ details }: { details: LogEntry['details'] }) => {
  ...
  if (details.type === 'table') {
    const data = toTableData(details.data);
    if (!data) return null;
    return (
          ...
          <div className="mb-2">
            ...
            <div className="flex flex-wrap gap-1">
              {data.channels.map((ch, i) => (
                <span
                  key={i}
                  className="px-1.5 py-0.5 bg-teal-900/30 text-teal-300 text-[10px] rounded border border-teal-800/50"
                >
                  {ch.name}
                </span>
              ))}
          ...
          <tbody className="text-[11px] font-mono text-neutral-300">
            {data.attributes.map((row, i) => (
              <tr key={i} className="border-b border-neutral-800/50">
                ...
                <td className="py-1 truncate max-w-[140px]" title={row.val}>
                  {row.val}
                </td>
              </tr>
            ))}
  ...
  if (details.type === 'chunk-map') {
    ...
    const data = toChunkMapData(details.data);
    if (!data) return null;
    const { total } = data;
    return (
      <div className="mt-2">
        ...
        <div className="flex h-2 w-full bg-neutral-800 rounded overflow-hidden">
          ...
          <div
            className="h-full bg-teal-500/80 w-full"
            title={`Read ${total} chunks successfully`}
          ></div>
        </div>
...
const LogItem: React.FC<{ log: LogEntry }> = ({ log }) => {
  ...
  return (
    <div className="border-l-2 border-neutral-700 ml-2 pl-4 pb-6 relative group">
      <div
        className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-neutral-900 bg-neutral-800 flex items-center justify-center ${
          log.status === LogStatus.Ok ? 'border-emerald-900/50' : ''
        }`}
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${
            log.status === LogStatus.Ok
              ? 'bg-emerald-500'
