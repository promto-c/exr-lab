  ...
  return (
    ...
    >
      ...
      >
        ...
        {part.type && (
          <span className="text-[10px] uppercase text-neutral-500 bg-neutral-900 px-1.5 py-0.5 rounded">
            {part.type.replace('image', '')}
          </span>
        )}
      </div>
      ...
      <div className="p-2 space-y-1">
        {Object.entries(layers).map(([layerName, fullChannelNames]) => {
          ...
          return (
            <div
              key={layerName}
              className="group flex flex-col items-start p-1.5 rounded hover:bg-neutral-700/30"
            >
              <div
              ...
              >
                ...
                <span
                  className="text-xs text-neutral-300 font-medium truncate group-hover:text-white transition-colors"
                  title={`Map ${layerName} to RGB`}
                >
                  {layerName}
                </span>
...
export const SidebarHeader: React.FC<SidebarHeaderProps> = ({ onOpenFile, onOpenFolder }) => {
  ...
  return (
    <div className="p-3 border-b border-neutral-800 shrink-0 flex items-center justify-between">
      <h1 className="text-sm font-bold flex items-center text-neutral-100">
        ...
        EXR Lab
        <span className="ml-2 text-[10px] bg-neutral-800 text-neutral-400 px-1.5 py-0 rounded">
          v{appVersion}
        </span>
        <a
...
export const StructurePanel: React.FC<StructurePanelProps> = ({
  ...
  return (
    <SubPanel
      ...
      className={cx(
        'rounded-lg border border-neutral-800 overflow-hidden min-h-0 flex-1',
        className,
      )}
