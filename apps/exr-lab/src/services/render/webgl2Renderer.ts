...
class WebGL2Renderer implements Renderer {
  ...
  public render(input: RenderFrameInput): RenderFrameOutput {
    ...
    const textures: Record<ChannelSlot, WebGLTexture> = {
      r: this.uploadChannelIfNeeded('r', rPlane, raw.width, raw.height)
        ? this.channelTextures.r
        : this.fallbackZeroTexture,
      g: this.uploadChannelIfNeeded('g', gPlane, raw.width, raw.height)
        ? this.channelTextures.g
        : this.fallbackZeroTexture,
      b: this.uploadChannelIfNeeded('b', bPlane, raw.width, raw.height)
        ? this.channelTextures.b
        : this.fallbackZeroTexture,
      a: this.uploadChannelIfNeeded('a', aPlane, raw.width, raw.height)
        ? this.channelTextures.a
        : this.fallbackOneTexture,
    };
    ...
    this.gl.uniform1f(
      this.resources.uniforms.invGamma,
      1.0 / (params.gamma > 0 ? params.gamma : 1.0),
    );
